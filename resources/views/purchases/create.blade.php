...
<div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Add New Purchase</h2>
    ...
    <form method="POST" action="{{ route('purchases.store') }}">
            ...
            <div>
                ...
                <select id="fish_category_id" name="fish_category_id" required onchange="updateCostPrice()"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" data-purchase-cost="{{ $category->purchase_cost }}">{{ $category->name }} - {{ $category->description }} (Cost: UGX {{ number_format($category->purchase_cost, 2) }})</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="invoice_number" name="invoice_number" value="{{ $invoiceNumber ?? '' }}" readonly
                   class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 bg-gray-100 leading-tight focus:outline-none focus:shadow-outline">
            <p class="text-xs text-gray-500 mt-1">Auto-generated if left empty (e.g., INV-2026-0001)</p>
        </div>
